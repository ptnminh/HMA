...
export class AuthController {
  ...
  @MessagePattern(AuthCommand.FIND_SCHEDULE_BY_ID)
  async findScheduleById (data: any) {
    try {
        const {id} = data
        const schedule = await this.authService.findScheduleById(id)
        if (!schedule) {
            return {
                status: HttpStatus.BAD_REQUEST,
...
@MessagePattern(AuthCommand.FIND_SCHEDULE_BY_USER_ID)
async findScheduleByUserId(data: any) {
    try {
        const {userId} = data
        const schedules = await this.authService.findScheduleByUserId(userId)
        return {
            status: HttpStatus.OK,
...
@MessagePattern(AuthCommand.CREATE_SCHEDULE)
async createSchedule(data: any) {
    try {
        const payload: Prisma.userSchedulesUncheckedCreateInput = {...data}
        console.log({...payload})
        const schedule = await this.authService.createSchedule(payload)
        if (!schedule) {
            return {
                status: HttpStatus.BAD_REQUEST,
...
@MessagePattern(AuthCommand.DELETE_SCHEDULE)
async deleteSchedule(data: any) {
    try {
        const {id} = data
        console.log(id)
        await this.authService.deleteSchedule(id)
        const schedule = await this.authService.findScheduleById(id)
        if (schedule) {
            return {
                status: HttpStatus.BAD_REQUEST,
...
@MessagePattern(AuthCommand.UPDATE_SCHEDULE)
async updateSchedule(data: any) {
    try {
        const {id, ...payload} = data
        const updateInput: Prisma.userSchedulesUncheckedUpdateInput = {...payload}
        const schedule = await this.authService.updateSchedule(id, updateInput)
        return  {
            status: HttpStatus.OK,
            data: schedule,
